...
async function parsePropertiesFromPayload(
  options: PayloadParsingOptions
): Promise<CustomValueMap> {
...
export async function getProjectData(
  ...
  for (const project of projects) {
    const columns =
      (await octokit.rest.projects.listColumns({ project_id: project.id }))
        .data || [];
    ...
    for (const column of columns) {
      const cards = (
        ...
        card =
          cards &&
          cards.find(
            (c) => Number(c.content_url?.split("/issues/")[1]) === issueNumber
          );
...
export async function run(options: Options) {
  ...
  if (github.payload.action === "opened") {
    ...
    if (!github.payload.repository?.full_name) {
      throw new Error(
        "Unable to find repository name in github webhook context"
      );
    }
